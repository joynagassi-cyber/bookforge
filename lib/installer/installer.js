...
`);
  ...
  const dirs = [
    ...
    'manifests',
    'bookforge-config/assets', 'bookforge-config/scripts'
  ];
  ...
  // Copy bookforge-config
  cp(path.join(root, 'bookforge', 'bookforge-config', 'config.yaml'), path.join(bf, 'bookforge-config', 'config.yaml'));
  cp(path.join(root, 'bookforge', 'bookforge-config', 'config.user.yaml'), path.join(bf, 'bookforge-config', 'config.user.yaml'));
  cp(path.join(root, 'bookforge', 'bookforge-config', 'module-help.csv'), path.join(bf, 'bookforge-config', 'module-help.csv'));
  cp(path.join(root, 'bookforge', 'bookforge-config', 'scripts', 'merge-config.py'), path.join(bf, 'bookforge-config', 'scripts', 'merge-config.py'));
  cp(path.join(root, 'bookforge', 'bookforge-config', 'scripts', 'cleanup-legacy.py'), path.join(bf, 'bookforge-config', 'scripts', 'cleanup-legacy.py'));
  ...
  // Also create manifests/workflows.json for CLI workflow commands
  write(path.join(bf, 'manifests', 'workflows.json'), JSON.stringify(workflowsManifest.workflows, null, 2) + '\n');
...
export function validate(projectDir) {
  ...
  const configPath = exists(path.join(projectDir, 'bookforge', 'bookforge-config', 'config.yaml'))
    ? 'bookforge/bookforge-config/config.yaml'
...
export function doctor(projectDir) {
  ...
  const checks = [
    ...
    { name: 'Configuration', check: () => exists(path.join(projectDir, 'bookforge', 'config.yaml')) || exists(path.join(projectDir, 'bookforge', 'bookforge-config', 'config.yaml')) },
...
export async function interactiveInstall(projectDir, options = {}) {
  ...
  console.log('\n' + '═'.repeat(60));
  console.log('  BookForge Installation Wizard');
  ...
  if (projectInput.trim()) config.project = path.resolve(projectInput.trim());

  // Book type
  console.log('\n📚 BOOK TYPE');
  const bookTypes = ['General Book', 'Fiction', 'Novel', 'Non-fiction', 'Technical Book', 'Academic', 'Memoir', 'Devotional'];
  bookTypes.forEach((bt, i) => console.log(`  ${i + 1}. ${bt}`));
  const bookInput = await ask('\nSelect book type [1-8]: ');
  const bookIdx = parseInt(bookInput) - 1;
  if (!isNaN(bookIdx) && bookIdx >= 0 && bookIdx < bookTypes.length) config.bookType = bookTypes[bookIdx];
  ...
  if (detectedHosts.length === 0) {
    ...
    console.log(`  Detected: ${detectedHosts.join(', ')}`);
    const hostChoices = ['Auto Detect (use all)', 'All Hosts', ...detectedHosts.map(h => formatHostName(h)), 'Generic'];
    hostChoices.forEach((hc, i) => console.log(`  ${i + 1}. ${hc}`));
    const hostInput = await ask('\nSelect host [1-' + hostChoices.length + ']: ');
    const hostIdx = parseInt(hostInput) - 1;
    if (hostIdx === 0) config.host = 'auto';
    else if (hostIdx === 1) config.host = 'all';
    else if (!isNaN(hostIdx) && hostIdx >= 2 && hostIdx < hostChoices.length) {
      const selected = hostChoices[hostIdx];
      const hostMap = { 'Claude Code': 'claude-code', 'Cursor': 'cursor', 'Windsurf': 'windsurf', 'Kiro': 'kiro', 'Codex CLI': 'codex-cli', 'Devin': 'devin', 'OpenCode': 'opencode', 'KiloCode': 'kilocode', 'Antigravity': 'antigravity', 'GitHub Copilot': 'github-copilot', 'Gemini': 'gemini', 'Generic': 'generic' };
      config.host = hostMap[selected] || 'generic';
    }
  }

  // Graph
  console.log('\n📊 GRAPH MEMORY');
  const graphOptions = ['JSONL (recommended)', 'Neo4j (requires setup)', 'None'];
  graphOptions.forEach((g, i) => console.log(`  ${i + 1}. ${g}`));
  const graphInput = await ask('\nSelect graph provider [1-3]: ');
  const graphIdx = parseInt(graphInput) - 1;
  if (!isNaN(graphIdx) && graphIdx >= 0 && graphIdx < 3) {
    const graphMap = { 0: 'jsonl', 1: 'neo4j', 2: 'none' };
    config.graph = graphMap[graphIdx];
  }
  ...
  modules.forEach(m => console.log(`  [${config.knowledge.includes(m.id) ? '✓' : ' '}] ${m.label}`));
  const knowledgeInput = await ask('\nSelect modules (comma-separated, e.g., core,writing,quality): ');
  if (knowledgeInput.trim()) config.knowledge = knowledgeInput.trim().split(',').map(s => s.trim()).filter(Boolean);

  // Example project
  console.log('\n📝 EXAMPLE PROJECT');
  const exampleInput = await ask('Include example/demo project? [y/N]: ');
  ...
  console.log(`  Book Type:  ${config.bookType}`);
  console.log(`  Host:       ${config.host}`);
  console.log(`  Graph:      ${config.graph}`);
  console.log(`  Knowledge:  ${config.knowledge.join(', ')}`);
  console.log(`  Example:    ${config.example ? 'Yes' : 'No'}`);
  console.log('═'.repeat(60) + '\n');
...
function formatHostName(host) {
  const names = {
    ...
    'kiro': 'Kiro', 'codex-cli': 'Codex CLI', 'devin': 'Devin',
    'opencode': 'OpenCode', 'kilocode': 'KiloCode', 'antigravity': 'Antigravity',
    'github-copilot': 'GitHub Copilot', 'gemini': 'Gemini', 'generic': 'Generic'
  };
