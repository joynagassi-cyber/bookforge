...
export async function initProject(project,{template='book',host='auto',graph='none'}={}){
  ...
  cp(path.join(root,'bookforge','_bmad','config.yaml'),path.join(project,'bookforge','_bmad','config.yaml'));
  cp(path.join(root,'bookforge','_bmad','config.user.yaml'),path.join(project,'bookforge','_bmad','config.user.yaml'));
  ...
  mkdir(path.join(project,'bookforge','_bmad','assets')); mkdir(path.join(project,'bookforge','_bmad','scripts'));
  cp(path.join(root,'bookforge','_bmad','assets','module.yaml'),path.join(project,'bookforge','_bmad','assets','module.yaml'));
  cp(path.join(root,'bookforge','_bmad','scripts','merge-config.py'),path.join(project,'bookforge','_bmad','scripts','merge-config.py'));
  cp(path.join(root,'bookforge','_bmad','scripts','cleanup-legacy.py'),path.join(project,'bookforge','_bmad','scripts','cleanup-legacy.py'));
  cp(path.join(root,'bookforge','_bmad','module-help.csv'),path.join(project,'bookforge','_bmad','module-help.csv'));
  ...
  console.log('Configuration template lives in bookforge/_bmad/. Run `bookforge setup` to configure interactively.');
}
...
export function validate(project){
 const required=['bookforge/project.json','bookforge/PROJECT-CONSTITUTION.md','bookforge/state/bookforge-state.md'];
 const configPath = fs.existsSync(path.join(project,'bookforge','_bmad','config.yaml'))
   ? 'bookforge/_bmad/config.yaml'
   : 'bookforge/config.yaml';
 required.push(configPath);
 const missing=required.filter(x=>!fs.existsSync(path.join(project,x))); if(missing.length){console.error('FAIL',missing); process.exitCode=2;} else console.log('PASS BookForge project contract');
}
