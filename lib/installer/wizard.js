...
function printBanner(version) {
  ...
  const tagPad = Math.floor((68 - tag.length) / 2);
  log(D.repeat(70));
  log(D + S.repeat(tagPad + 1) + C.bold + C.cyan + tag + C.reset + S.repeat(68 - tagPad - tag.length) + D);
  log(D.repeat(70));

  const ver = '  v' + version + '  |  Build More, Write Smarter  |  (c) BookForge  ';
  log(D + S.repeat(tagPad + 1) + C.dim + ver + C.reset + S.repeat(68 - tagPad - ver.length) + D);
  log(D.repeat(70));
  log('');
}
// ============================================================
