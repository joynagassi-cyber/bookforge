...
const C = {
  ...
  amber: '\x1B[38;5;220m',
  slate: '\x1B[38;5;243m',
  slateLight: '\x1B[38;5;248m',
  slateDark: '\x1B[38;5;235m',
};
...
function printStepHeader(current, total, title, subtitle) {
  ...
  clearScreen();
  console.log(C.dim + '  BookForge Setup Wizard'.padEnd(50) + C.reset + `  ${current}/${total}`);
  console.log(C.dim + '  ' + '─'.repeat(70) + C.reset);
  console.log(C.bold + C.indigo + `  ${bar}  ${progress}%` + C.reset);
...
async function stepHost() {
  ...
  if (detected.length > 0) {
    ...
    detected.forEach(h => console.log(C.slateLight + `    • ${h}` + C.reset));
    console.log();
  }
