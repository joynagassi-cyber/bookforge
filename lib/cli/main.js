...
const projectDir = (args) => {
  const pos = args.filter(x => !x.startsWith('--') && x !== 'status' && x !== 'validate' && x !== 'doctor' && x !== 'install' && x !== 'init' && x !== 'wizard' && x !== 'plugin' && x !== 'host' && x !== 'graph' && x !== 'route' && x !== 'workflow' && x !== 'context-pack' && x !== 'catalog-search' && x !== 'watch' && x !== 'graph-sync');
...
export async function main(args) {
  ...
  switch (cmd) {
    ...
    case 'workflow': {
      ...
      const project = projectDir(args);
      if (sub === 'plan') {
        const task = positionals(args).filter(x => x !== 'workflow' && x !== 'plan').slice(1).join(' ');
        return console.log(JSON.stringify(await planWorkflow(project, args[2], {
          task,
          agent: arg(args, '--agent'),
          genre: arg(args, '--genre'),
          bookType: arg(args, '--book-type'),
